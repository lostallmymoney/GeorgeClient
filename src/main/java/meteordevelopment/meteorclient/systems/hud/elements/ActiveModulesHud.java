...
public class ActiveModulesHud extends HudElement {
    ...
    public void tick(HudRenderer renderer) {
        ...
        for (Module module : Modules.get().getActive()) {
            if (hiddenModules.get().contains(module)) continue;
            for (String title : module.getHudTitles()) {
                if (title == null || title.isBlank()) continue;
                modules.add(new ModuleEntry(module, title));
            }
        }
        ...
        for (ModuleEntry entry : modules) {
            width = Math.max(width, getModuleWidth(renderer, entry));
    ...
    private void renderModule(HudRenderer renderer, int index, double x, double y) {
        ModuleEntry entry = modules.get(index);
        Module module = entry.module;
        String moduleTitle = entry.title;
        ...
        renderer.text(moduleTitle, x, y, color, shadow.get(), getScale());

        double textHeight = renderer.textHeight(shadow.get(), getScale());
        double textLength = renderer.textWidth(moduleTitle, shadow.get(), getScale());
        ...
        if (activeInfo.get()) {
            String info = module.getInfoString(moduleTitle);
            if (info != null) {
    ...
    private double getModuleWidth(HudRenderer renderer, ModuleEntry entry) {
        Module module = entry.module;
        String moduleTitle = entry.title;
        if (moduleTitle.isBlank()) return 0;
        double width = renderer.textWidth(moduleTitle, shadow.get(), getScale());
    ...
    private record ModuleEntry(Module module, String title) {}
