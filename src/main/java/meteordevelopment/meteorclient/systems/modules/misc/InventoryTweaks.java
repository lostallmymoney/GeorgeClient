...
public class InventoryTweaks extends Module {
    ...
    private final Setting<InventorySorter.SortMode> sortingMode = sgSorting.add(new EnumSetting.Builder<InventorySorter.SortMode>()
        .name("sorting-mode")
        .description("How item stacks are ordered while sorting. ItemType uses Inventive Inventory ordering.")
        .defaultValue(InventorySorter.SortMode.ItemType)
        .visible(sortingEnabled::get)
        .build()
    );
    ...
    private final Setting<Boolean> rapidContainerSorting = sgSorting.add(new BoolSetting.Builder()
        .name("rapid-container-sorting")
        .description("Runs sorting faster in container inventories by performing multiple swaps per tick.")
        .defaultValue(true)
        .visible(sortingEnabled::get)
        .build()
    );

    private final Setting<Integer> containerActionsPerTick = sgSorting.add(new IntSetting.Builder()
        .name("container-actions-per-tick")
        .description("How many item swaps to execute per tick while sorting container inventories.")
        .defaultValue(6)
        .min(1)
        .sliderRange(1, 20)
        .visible(() -> sortingEnabled.get() && rapidContainerSorting.get())
        .build()
    );
    ...
    private boolean sort() {
        ...
        sorter = new InventorySorter(screen, focusedSlot, sortingMode.get());
        return true;
    }
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        int delay = sortingDelay.get();
        int actionsPerTick = 1;

        if (rapidContainerSorting.get() && sorter.isContainerSort()) {
            delay = 0;
            actionsPerTick = containerActionsPerTick.get();
        }

        if (sorter.tick(delay, actionsPerTick)) sorter = null;
    }
