...
public class AutoFish extends Module {
    ...
    private final Setting<Boolean> allowOffhand = sgGeneral.add(new BoolSetting.Builder()
        .name("allow-offhand")
        .description("Allows fishing using a rod in your offhand.")
        .defaultValue(false)
        .build()
    );
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (rodHand == null && autoSwitch.get() && bestRodSlot != -1 && mc.player.getInventory().getSelectedSlot() != bestRodSlot) {
    ...
    private void tryCast(Hand hand) {
    ...
    private void tryCatch(Hand hand) {
        if (mc.player.fishHook == null) return;
    ...
    private Hand getRodHand() {
        if (allowOffhand.get() && isUsableRod(mc.player.getOffHandStack())) return Hand.OFF_HAND;
        if (isUsableRod(mc.player.getMainHandStack())) return Hand.MAIN_HAND;
        return null;
    }

    private boolean isUsableRod(ItemStack stack) {
        if (!(stack.getItem() instanceof FishingRodItem)) return false;
        return !antiBreak.get() || stack.getDamage() < stack.getMaxDamage() - 1;
    }
