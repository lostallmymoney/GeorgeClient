...
public class AddonManager {
    ...
    private static final String LEGACY_COLOR_KEY = MeteorClient.LEGACY_MOD_ID + ":color";
    ...
    public static void init() {
        // George pseudo addon
        {
            ...
            if (metadata.containsCustomValue(MeteorClient.MOD_ID + ":color")) MeteorClient.ADDON.color.parse(metadata.getCustomValue(MeteorClient.MOD_ID + ":color").getAsString());
        ...
        for (EntrypointContainer<MeteorAddon> entrypoint : FabricLoader.getInstance().getEntrypointContainers("meteor", MeteorAddon.class)) {
            ...
            if (metadata.containsCustomValue(MeteorClient.MOD_ID + ":color")) addon.color.parse(metadata.getCustomValue(MeteorClient.MOD_ID + ":color").getAsString());
            else if (metadata.containsCustomValue(LEGACY_COLOR_KEY)) addon.color.parse(metadata.getCustomValue(LEGACY_COLOR_KEY).getAsString());
