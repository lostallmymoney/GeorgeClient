...
public class InvUtils {
    private static final int PICKUP_LEFT = 0;
    private static final int PICKUP_RIGHT = 1;
    ...
    public static boolean innovativeMove(int from, int to) {
        return innovativeMoveById(SlotUtils.indexToId(from), SlotUtils.indexToId(to));
    }

    public static boolean innovativeMoveById(int fromId, int toId) {
        ...
        pickupClick(fromId);
        pickupClick(toId);
        if (!mc.player.currentScreenHandler.getCursorStack().isEmpty()) pickupClick(fromId);

        boolean success = mc.player.currentScreenHandler.getCursorStack().isEmpty();
        return success;
    }

    private static void pickupClick(int slotId) {
        ItemStack cursorStack = mc.player.currentScreenHandler.getCursorStack();
        ItemStack slotStack = slotId >= 0 && slotId < mc.player.currentScreenHandler.slots.size()
            ? mc.player.currentScreenHandler.getSlot(slotId).getStack()
            : ItemStack.EMPTY;

        int clickType = (cursorStack.getItem() instanceof BundleItem && !slotStack.isEmpty())
            || (slotStack.getItem() instanceof BundleItem && !cursorStack.isEmpty())
            ? PICKUP_RIGHT
            : PICKUP_LEFT;
