...
public class SettingCommand extends Command {
                        ...
                        argument("value", SettingValueArgumentType.create())
                            .executes(context -> {
                                ...
                                if (setting.parse(value)) {
                                    ChatUtils.infoPrefix("Config", "Setting (highlight)%s(default) %s (highlight)%s(default).", setting.title, getChangeVerb(setting, value), setting.get());
                                }
                    ...
                    .then(
                        argument("value", SettingValueArgumentType.create())
                            .executes(context -> {
                                ...
                                if (setting.parse(value)) {
                                    ModuleArgumentType.get(context).info("Setting (highlight)%s(default) %s (highlight)%s(default).", setting.title, getChangeVerb(setting, value), setting.get());
                                }
    ...
    private static String getChangeVerb(Setting<?> setting, String input) {
        if (input.equalsIgnoreCase("toggle") && setting instanceof BoolSetting) return "toggled to";
        if (input.equalsIgnoreCase("cycle") && setting instanceof CycleSetting<?>) return "cycled to";
        return "changed to";
    }
}
