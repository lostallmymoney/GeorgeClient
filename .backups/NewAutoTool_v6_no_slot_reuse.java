...
public class NewAutoTool extends Module {
    ...
    private final Set<Integer> usedBorrowSlots = new HashSet<>();  // Track which slots were borrowed
    ...
    private void switchToTool(ToolSlot tool) {
        ...
        if (tool.source == ToolSource.HOTBAR) {
            ...
            if (inventorySwitchBack.get()) {
                ...
                // Mark this slot as borrowed
                usedBorrowSlots.add(borrowSlot);
    ...
    private int findFreeBorrowSlot() {
        // Prefer empty slots that haven't been borrowed yet
        for (int i = HOTBAR_START; i <= HOTBAR_END; i++) {
            if (!usedBorrowSlots.contains(i) && mc.player.getInventory().getStack(i).isEmpty()) {
                return i;
            }
        }

        // Use non-protected, non-borrowed slots
        for (int i = HOTBAR_START; i <= HOTBAR_END; i++) {
            if (!usedBorrowSlots.contains(i)) {
                ItemStack stack = mc.player.getInventory().getStack(i);
        ...
        // If forced to reuse, prefer already-borrowed slots over protected ones
        for (int i = HOTBAR_START; i <= HOTBAR_END; i++) {
            if (usedBorrowSlots.contains(i)) {
                ItemStack stack = mc.player.getInventory().getStack(i);
                if (!isProtected(stack)) {
